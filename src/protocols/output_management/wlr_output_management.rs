...
where
    Self: GlobalDispatch<ZwlrOutputManagerV1, Box<dyn for<'c> Fn(&'c Client) -> bool + Send + Sync>>
        ...
        + Dispatch<ZwlrOutputHeadV1, WlrHeadData>
...
#[derive(Clone)]
struct WlrHeadDataInner {
    output: WeakOutput,
    instances: Arc<Mutex<Vec<ZwlrOutputHeadV1>>>,
    modes: Arc<Mutex<Vec<WlrMode>>>,
}

#[derive(Clone)]
pub struct WlrHeadData(WlrHeadDataInner);
...
impl WlrOutputManagementState {
    ...
    pub(super) fn cur_config_serial(&self) -> Serial {
        self.cur_config_serial
    }

    pub fn output_created<H: WlrOutputManagementHandler>(&mut self, output: &Output) {
        self.output_created_return_serial::<H>(output);
    }

    pub(super) fn output_created_return_serial<H: WlrOutputManagementHandler>(&mut self, output: &Output) -> Serial {
    ...
    pub fn output_changed<H: WlrOutputManagementHandler>(&mut self, output: &Output, is_enabled: bool) {
        self.output_changed_return_serial::<H>(output, is_enabled);
    }

    pub(super) fn output_changed_return_serial<H: WlrOutputManagementHandler>(
        &mut self,
        output: &Output,
        is_enabled: bool,
    ) -> Option<Serial> {
    ...
    pub fn output_destroyed(&mut self, output: &Output) {
        self.output_destroyed_return_serial(output);
    }

    pub(super) fn output_destroyed_return_serial(&mut self, output: &Output) -> Option<Serial> {
...
impl WlrHead {
    ...
    pub(super) fn data(&self) -> WlrHeadData {
        self.data.clone()
    }

    fn instances(&self) -> MutexGuard<'_, Vec<ZwlrOutputHeadV1>> {
        self.data.0.instances.lock().unwrap()
    }

    fn modes(&self) -> MutexGuard<'_, Vec<WlrMode>> {
        self.data.modes()
    }
        ...
        if let Some(diff) = OutputDiff::new(output, is_enabled, self) {
            ...
            if !diff.modes_removed.is_empty() {
                for mode in diff.modes_removed {
                    if let Some(pos) = self.modes().iter().position(|wlr_mode| wlr_mode.mode == mode) {
                        let mode = self.modes().remove(pos);
...
impl WlrHeadData {
    fn new(output: &Output) -> Self {
        Self(WlrHeadDataInner {
            instances: Arc::new(Mutex::new(Vec::new())),
            output: output.downgrade(),
            modes: Arc::new(Mutex::new(
                output
                    .modes()
                    .into_iter()
                    .map(|mode| WlrMode {
...
impl OutputDiff {
    fn new(output: &Output, is_enabled: bool, head: &WlrHead) -> Option<Self> {
        let (modes_added, modes_removed) = {
            let head_modes = head.modes();

            let mut modes_added = output.modes();
            modes_added.retain(|new_mode| !head_modes.iter().any(|old_mode| old_mode.mode == *new_mode));

            let mut modes_removed = head_modes.iter().map(|mode| mode.mode).collect::<Vec<_>>();
            modes_removed.retain(|old_mode| !output.modes().iter().any(|new_mode| old_mode == new_mode));

            (modes_added, modes_removed)
        };
...
impl<H: WlrOutputManagementHandler> Dispatch<ZwlrOutputHeadV1, WlrHeadData, H> for WlrOutputManagementState {
    fn request(
        state: &mut H,
        client: &Client,
        ...
        if let Request::Release = request {
            <Self as Dispatch<ZwlrOutputHeadV1, WlrHeadData, H>>::destroyed(state, client.id(), resource, data)
        }
    }

    fn destroyed(_state: &mut H, _client: ClientId, resource: &ZwlrOutputHeadV1, data: &WlrHeadData) {
        data.0.instances.lock().unwrap().retain(|instance| instance != resource);
    }
}
        ...
        match request {
            Request::EnableHead { id, head } => {
                ...
                if let Some(config) = state.configurations.iter_mut().find(|config| &config.instance == resource)
                    && let Some((output, allowed_modes)) = head
                        .data::<WlrHeadData>()
                        .and_then(|data| data.output().map(|output| (output, data.modes().clone())))
                {
                    if config.used {
                        resource.post_error(Error::AlreadyUsed, "this configuration has already been tested or applied");
                    } else if config.update_for_output(&output).is_some() {
                        resource.post_error(
            ...
            Request::DisableHead { head } => {
                ...
                if let Some(config) = state.configurations.iter_mut().find(|config| &config.instance == resource)
                    && let Some(output) = head.data::<WlrHeadData>().and_then(|data| data.output())
                {
                    if config.used {
                        resource.post_error(Error::AlreadyUsed, "this configuration has already been tested or applied");
                    } else if config.update_for_output(&output).is_some() {
...
impl<H: WlrOutputManagementHandler> Dispatch<ZwlrOutputConfigurationHeadV1, (), H> for WlrOutputManagementState {
    fn request(
        ...
        // NB: since all this is shared with XfceOutputManagementState's WlrHead instances, do not
        // ever look at anything but 'configurations' from WlrOutputManagementState.  For example,
        // if you try to look at 'heads', you might not find the one you are looking for because
        // it's stored on XfceOutputManagementState, not WlrOutputManagementState.
        let configurations = &mut state.wlr_output_management_state().configurations;

        if let Some(config) = configurations.iter_mut().find(|config| {
            config
                .updates
                .iter()
        ...
        }) && config.used
        {
        ...
        } else if let Some(config_head) = configurations.iter_mut().find_map(|config| {
            config
                .updates
                .iter_mut()
            ...
            match request {
                Request::SetMode { mode } => {
                    ...
                    } else if let Some(mode) = config_head
                        .allowed_modes
                        .iter()
...
fn create_and_send_head<H: WlrOutputManagementHandler>(
    ...
    let instance = client.create_resource::<ZwlrOutputHeadV1, _, H>(dh, manager_instance.version(), head.data())?;
...
macro_rules! delegate_wlr_output_management {
    ($(@<$( $lt:tt $( : $clt:tt $(+ $dlt:tt )* )? ),+>)? $ty: ty) => {
        ...
        smithay::reexports::wayland_server::delegate_dispatch!($(@< $( $lt $( : $clt $(+ $dlt )* )? ),+ >)? $ty: [
            smithay::reexports::wayland_protocols_wlr::output_management::v1::server::zwlr_output_head_v1::ZwlrOutputHeadV1: $crate::protocols::output_management::wlr_output_management::WlrHeadData
        ] => $crate::protocols::output_management::wlr_output_management::WlrOutputManagementState);
        smithay::reexports::wayland_server::delegate_dispatch!($(@< $( $lt $( : $clt $(+ $dlt )* )? ),+ >)? $ty: [
