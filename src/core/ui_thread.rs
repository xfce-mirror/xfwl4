    ...
    fn handle_window_menu_action(&mut self, window: WindowElement, action: WindowMenuAction) {
        match action {
            ...
            WindowMenuAction::Resize => {
                if let Some(keyboard) = self.core.seat.get_keyboard()
                    && let Some(window_loc) = self.core.workspace_manager.active_workspace().element_location(&window)
                {
                    self.warp_pointer_to_resize_edge(&window, window_loc, ResizeEdge::BOTTOM_RIGHT);
                    let serial = SERIAL_COUNTER.next_serial();
                    keyboard.set_focus(self, Some(window.clone().into()), serial);
