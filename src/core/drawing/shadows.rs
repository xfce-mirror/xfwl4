...
pub fn make_shadow(
    ...
    let swidth = size.w + gaussian_size - delta_width - delta_loc.x;
    let sheight = size.h + gaussian_size - delta_height - delta_loc.y;
...
pub fn make_shadow_texture(
    ...
    let Some((data, swidth, sheight)) = make_shadow(opacity, size, delta_loc, delta_width, delta_height) else {
        return Ok(None);
    };
