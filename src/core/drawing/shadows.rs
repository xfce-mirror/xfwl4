...
impl ShadowParams {
    pub fn new(delta_x: i32, delta_y: i32, delta_width: i32, delta_height: i32, surface_w: i32, surface_h: i32) -> Self {
        let gaussian_size = ((SHADOW_RADIUS * 3.).ceil() as i32 + 1) & !1;
        ShadowParams {
            offset_x: SHADOW_OFFSET_X + delta_x,
            offset_y: SHADOW_OFFSET_Y + delta_y,
            shadow_width: surface_w + gaussian_size - delta_width - delta_x,
            shadow_height: surface_h + gaussian_size - delta_height - delta_y,
        }
    }
}
...
pub fn make_shadow(
    ...
    let swidth = width + gaussian_size - delta_width - delta_x;
    let sheight = height + gaussian_size - delta_height - delta_y;
    ...
    let d = if gaussian_size > 0 {
        ...
        sum_gaussian(&map, opacity, center, center, width, height)
    };
    ...
    for y in 0..ylimit {
        for x in 0..xlimit {
            let d = if xlimit == gaussian_size && ylimit == gaussian_size {
                ...
                sum_gaussian(&map, opacity, x - center, y - center, width, height)
            };
    ...
    if x_diff > 0 && ylimit > 0 {
        for y in 0..ylimit {
            let d = if ylimit == gaussian_size {
                ...
                sum_gaussian(&map, opacity, center, y - center, width, height)
            };
            for i in 0..x_diff {
    ...
    for x in 0..xlimit {
        let d = if xlimit == gaussian_size {
            ...
            sum_gaussian(&map, opacity, x - center, center, width, height)
        };
    ...
    Some((img.data, swidth, sheight))
}

#[allow(clippy::too_many_arguments)]
pub fn make_shadow_texture(
    ...
    delta_width: i32,
    delta_height: i32,
) -> anyhow::Result<Option<(GlesTexture, i32, i32)>> {
    let Some((data, swidth, sheight)) = make_shadow(opacity, width, height, delta_x, delta_y, delta_width, delta_height) else {
        return Ok(None);
    };

    let rgba: Vec<u8> = data.iter().flat_map(|&alpha| [0, 0, 0, alpha]).collect();

    let size: Size<i32, Buffer> = (swidth, sheight).into();
    let texture = renderer
        .import_memory(&rgba, Fourcc::Abgr8888, size, false)
        .map_err(|err| anyhow::anyhow!("{err}"))?;

    Ok(Some((texture, swidth, sheight)))
}
