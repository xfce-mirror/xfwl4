...
pub fn read_until<FD: AsFd>(fd: FD, buf: &mut [u8], ends_with: &[u8]) -> Result<usize, Errno> {
    let mut total_len = 0;
    loop {
        match rustix::io::read(&fd, &mut buf[total_len..(total_len + 1)]) {
            ...
            Ok(0) => break Err(Errno::NODATA),
            Ok(len) if total_len + len == buf.len() => break Err(Errno::NOSPC),
...
pub fn read_exact<FD: AsFd>(fd: FD, buf: &mut [u8]) -> Result<usize, Errno> {
    let mut total_len = 0;
    loop {
        match rustix::io::read(&fd, &mut buf[total_len..]) {
            Err(errno) if errno == Errno::INTR || errno == Errno::AGAIN => (),
            Err(errno) => break Err(errno),
            Ok(len) if len == 0 && total_len < buf.len() => break Err(Errno::NODATA),
